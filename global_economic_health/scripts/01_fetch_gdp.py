...
Fetch nominal GDP (NY.GDP.MKTP.CD) from World Bank API for all countries, 1960-2026.
Output: CSV file with country_name, country_code, year, gdp_usd columns.

Data Source: World Bank Open Data
Indicator: NY.GDP.MKTP.CD (GDP at market prices, current USD)
"""
...
def fetch_worldbank_gdp(
    ...
    Args:
        indicator: World Bank indicator code (default: NY.GDP.MKTP.CD)
        start_year: Start year for data fetch
        end_year: End year for data fetch
        config: Configuration dictionary with API settings
    
    Returns:
        DataFrame with columns: country_name, country_code, year, gdp_usd
    ...
    logger.info("=" * 70)
    logger.info("FETCHING WORLD BANK GDP DATA")
    logger.info("=" * 70)
    logger.info(f"[FETCH] Requesting data from World Bank API...")
            ...
            for record in records:
                if record.get("value") is not None:
                    rows.append({
                        ...
                        "gdp_usd": float(record.get("value"))
                    })
    ...
    raise Exception(f"Failed to fetch GDP data after {max_retries} attempts")
...
def validate_response(response: requests.Response) -> None:
    ...
    logger.info(f"[VALIDATION] Response valid: HTTP {response.status_code}, content-type: {content_type}")
...
def main():
    ...
    output_file = Path(__file__).parent.parent / 'csv' / 'raw' / f'gdp_raw_{timestamp}.csv'
    
    try:
        df = fetch_worldbank_gdp()
        save_to_csv(df, output_file)
        
        logger.info("=" * 70)
        logger.info(f"✅ COMPLETE: GDP data fetched successfully")
